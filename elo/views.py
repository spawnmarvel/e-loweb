    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if option == "title is":
                ...
                get_note = Note.objects.filter(
                    Q(raw_text__icontains=str(inp_text)))
                ...
                hook = "wtf"
            comment = "Started search with type: " + \
                format(hook) + ":  for input: " + inp_text
            data = {"comment": comment, "hook": hook,
                    "note": get_note, "form": form}
...
def insert_db(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            note = Note(
                title=inp_title, raw_text=tu[0], sentences=tu[1], words=tu[2], hook=inp_hook)
            note.save()
